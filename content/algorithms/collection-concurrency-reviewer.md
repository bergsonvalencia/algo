...
**Concurrency** is two or more operations executing at the same time. The classic flavor is *multiple threads inside one process* sharing a mutable [collection](algorithms-glossary-reviewer.md#collection "An object that groups multiple elements, like a list, set, queue, or dictionary.") — a shared `List<T>`, a `Dictionary<K,V>`, a `PriorityQueue<T>` — but the same hazard appears one tier up (multiple processes sharing a file) and one tier up again (multiple machines sharing a database table). The unit of trouble is always the same: **shared mutable state touched without coordination**. The standard library collections (`List<T>`, `Dictionary<TKey,TValue>`, `Queue<T>`, `Stack<T>`) are explicitly *not* [thread-safe](algorithms-glossary-reviewer.md#thread-safe "A type whose operations behave correctly when called concurrently from multiple threads.") — they assume a single writer, and concurrent writers silently corrupt them.

The danger is not theoretical. A single logical operation like "enqueue" or "increment a count" compiles into several memory reads and writes; when two threads interleave those steps, you get **[race conditions](algorithms-glossary-reviewer.md#race-condition "A bug where the result depends on the nondeterministic timing of threads accessing shared state.")**, **[torn reads](algorithms-glossary-reviewer.md#torn-read "A read that observes a value partway through another thread's write, seeing an inconsistent mix of old and new bits.")**, [lost updates](algorithms-glossary-reviewer.md#lost-update "When one thread's write is silently overwritten by another because both read the same starting value."), and shattered [invariants](algorithms-glossary-reviewer.md#invariant "A condition that stays true at every step, used to prove correctness.") (a heap that no longer satisfies the heap property, a [linked list](linked-lists-reviewer.md) whose `Count` lies about its length). This reviewer pins down *why* unsynchronized collections break, then walks the three locking strategies in order of sophistication — **caller synchronization**, internal **[monitor locking](algorithms-glossary-reviewer.md#monitor-lock "A mutual-exclusion lock that lets only one thread at a time enter a protected region.")**, and **[reader/writer locking](algorithms-glossary-reviewer.md#reader-writer-lock "A lock that permits many concurrent readers or a single exclusive writer, favoring read-heavy workloads.")** — before turning to .NET's purpose-built `System.Collections.Concurrent` types and the rule that survives all of them: even a thread-safe collection cannot make a **[check-then-act](algorithms-glossary-reviewer.md#check-then-act "A non-atomic pattern where you test a condition and then act on it, leaving a gap another thread can exploit.")** sequence atomic for you.
...
- A collection holds an **invariant** that several fields must agree on: a `List<T>` keeps `_items`, `_size`, and `_version` consistent; a [binary heap](algorithms-glossary-reviewer.md#binary-heap "A complete binary tree stored in an array where each parent is ordered relative to its children.") keeps every parent ≤ its children; a `Dictionary<K,V>` keeps bucket pointers coherent. A write touches *several* of those fields, and the write is **not** a single indivisible step.
...
- A **[data race](algorithms-glossary-reviewer.md#data-race "Two threads accessing the same memory location concurrently, at least one writing, with no synchronization between them.")** specifically is two threads accessing the same location, at least one writing, with no synchronization ordering them. The fix is to establish a **[happens-before](algorithms-glossary-reviewer.md#happens-before "An ordering guarantee that one operation's effects are visible to another, established by synchronization.")** ordering — that is what every lock below provides.
...
- **Do as little as possible inside the lock.** Notice `Process()` runs *outside* the lock above — holding a lock during slow work serializes everything and kills throughput, and worse, calling unknown code while holding a lock invites **[deadlock](algorithms-glossary-reviewer.md#deadlock "A standstill where two or more threads each wait forever on a lock the other holds.")**.
...
- **[Lock-free](algorithms-glossary-reviewer.md#lock-free-algorithm "An algorithm guaranteeing system-wide progress without mutual exclusion, so no thread can block the others by holding a lock.")** algorithms make progress without mutual exclusion, typically built on an atomic **[compare-and-swap (CAS)](algorithms-glossary-reviewer.md#compare-and-swap "An atomic instruction that updates a value only if it still equals an expected one, retrying otherwise.")** — read a value, compute a new one, and atomically swap it in *only if it hasn't changed*; retry if it has. No thread can block the others by holding a lock, so one stalled thread can't freeze the system.
...
- In .NET you rarely hand-roll either: **`Interlocked`** gives you [atomic](algorithms-glossary-reviewer.md#atomic-operation "An operation that completes as a single indivisible step — other threads see it as either fully done or not started.") single-variable operations (`Increment`, `Add`, `Exchange`, `CompareExchange`), and the **concurrent collections** below package lock-free or fine-grained-locking implementations behind ordinary-looking method calls.
...
*Prefer immutability, then [Interlocked](algorithms-glossary-reviewer.md#interlocked "A .NET class providing atomic operations on a single variable, such as Increment and CompareExchange.") for one variable, then a concurrent collection, and reach for explicit locks only when an invariant spans multiple operations.*
...
- **A custom collection or an invariant spanning several operations → explicit locks**: a plain `lock` for balanced loads, `ReaderWriterLockSlim` only when reads truly dominate. Keep [critical sections](algorithms-glossary-reviewer.md#critical-section "A region of code that accesses shared state and must be executed by only one thread at a time.") tiny and never call out to unknown code while holding a lock (deadlock risk).
