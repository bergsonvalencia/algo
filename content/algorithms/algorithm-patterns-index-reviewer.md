...
[AlgoMonster](https://algo.monster/flowchart) publishes a widely-shared decision tree that, like the cue
sheet above, turns a problem statement into a recommended technique — but as a single top-down tree you walk
one yes/no answer at a time. Rather than inline its 91 nodes here, the full chart lives in its own reviewer,
with a navigation guide, a leaf→reviewer map, and worked traversals:
...
exhaustive walk for when no single cue jumps out.
