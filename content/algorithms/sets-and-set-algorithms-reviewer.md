...
A set is an [abstract data type](algorithms-glossary-reviewer.md#abstract-data-type "A model defined by its operations and behavior, independent of implementation.") — a contract of behavior, not a specific layout — modeling the mathematical notion of a set. It is the [collection](algorithms-glossary-reviewer.md#collection "A container holding multiple elements, accessed and iterated through its operations.") of **unique values, in no particular order**.
...
- **[Subset](algorithms-glossary-reviewer.md#subset "A selection of elements from a set; A is a subset of B if every element of A is in B."):** A ⊆ B means every item of A is in B. Test it by checking that `B.Contains(x)` holds for every `x` in A — **O(|A| · cost-of-membership)**. A **proper** (strict) subset additionally requires `|A| < |B|`.
